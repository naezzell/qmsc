...
def main(test, ansatz, input_list):
    ...
    for inp in input_list:
        ...
        with open(job_file, 'w') as fh:
            ...
            if ansatz == "vspa":
                fh.write(f"python {os.getcwd()}/../qmsc_vspa_script.py '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s'" % tuple(inp[0:11]))
            elif ansatz == "ccps":
                fh.write(f"python {os.getcwd()}/../qmsc_ccps_script.py '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s'" % tuple(inp[0:11]))
